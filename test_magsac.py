...
def run_magsac(pair, scene_scale, matches, relative_pose, image_size1, image_size2, K1, K2, args):
    ...
    H_est, inliers = pymagsac.findHomography(
        ...
        image_size1[0][0], image_size1[1][0], 
        image_size2[0][0], image_size2[1][0],
        probabilities = probabilities,
        conf = args.confidence,
        sampler = args.sampler,
        min_iters = args.minimum_iterations,
        max_iters = args.maximum_iterations,
        use_magsac_plus_plus = args.use_magsac_plus_plus,
...
def estimate_homographies(pairs, data, scene_scale, args):
    ...
    results = Parallel(n_jobs=min(args.core_number, len(pairs)))(delayed(run_magsac)(
...
if __name__ == "__main__":
    ...
    if args.use_magsac_plus_plus:
        print(f"Running MAGSAC++ on the '{split}' split of HEB")
    else:
        print(f"Running MAGSAC on the '{split}' split of HEB")
