...
def run_kornia_ransac(pair, scene_scale, matches, relative_pose, image_size1, image_size2, K1, K2, deep_confidence, args):
    ...
    x1y1 = matches[snn_mask, 0:2].astype(np.float32)
    x2y2 = matches[snn_mask, 2:4].astype(np.float32)
    ...
    ransac = K.geometry.RANSAC('homography',  args.inlier_threshold, batch_size, batched_max_iter, args.confidence, max_lo_iters=10).to(device)
    xy1 = torch.from_numpy(x1y1).to(device=device, dtype=torch.float32)
    xy2 = torch.from_numpy(x2y2).to(device=device, dtype=torch.float32)
    tic = time.perf_counter()
    if args.device =='cuda':
        torch.cuda.synchronize()
    with torch.inference_mode():
        H_est, inliers = ransac(xy1, xy2)
...
def estimate_homographies(pairs, data, scene_scale,deep_confidence_h5, args):
    ...
    results = Parallel(n_jobs=min(args.core_number, len(pairs)))(delayed(run_kornia_ransac)(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--batch_size", type=float, default=1024, help='kornia runs estimation in batches')
    ...
    print(f"Running LO-RANSAC (kornia) on the '{split}' split of HEB")
