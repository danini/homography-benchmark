...
def run_vsac(pair, scene_scale, matches, relative_pose, image_size1, image_size2, K1, K2, deep_confidence, args):
    ...
    sampling_method_p = pvsac.SamplingMethod.SAMPLING_UNIFORM
    ...
    if args.sampler != 'Uniform':
        ...
        sampling_method_p = pvsac.SamplingMethod.SAMPLING_PROSAC

    if args.score_type == 'msac':
        score_type_p = pvsac.ScoreMethod.SCORE_METHOD_MSAC
    elif args.score_type == 'magsac':
        score_type_p = pvsac.ScoreMethod.SCORE_METHOD_MAGSAC
    else:
    ...
    x2y2 = correspondences[:, 2:4]
    
    # Run the homography estimation implemented in VSAC
    tic = time.perf_counter()
    H_est, inliers = pvsac.estimate(params, x1y1, x2y2, None, None, None, None)
...
def estimate_homographies(pairs, data, scene_scale,deep_confidence_h5, args):
    ...
    results = Parallel(n_jobs=min(args.core_number, len(pairs)))(delayed(run_vsac)(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--score_type", type=str, help="Choose from: magsac, msac", choices=["magsac", "msac"], default="msac")
    ...
    print(f"Running VSAC on the '{split}' split of HEB")
